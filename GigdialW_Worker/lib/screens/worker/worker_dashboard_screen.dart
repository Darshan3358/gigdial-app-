...
class _WorkerDashboardScreenState extends State<WorkerDashboardScreen> {
                        ...
                        children: [
                          if (notifications.any((n) => n['read'] == false))
                            TextButton(
                              ...
                              child: const Text('Mark all read', style: TextStyle(color: Colors.teal, fontSize: 13)),
                            ),
                          TextButton(
                            ...
                            child: ListView.separated(
                              ...
                              itemBuilder: (context, index) {
                                ...
                                return InkWell(
                                  ...
                                  child: Container(
                                    color: isUnread ? Colors.teal.withOpacity(0.03) : Colors.transparent,
  ...
  Widget _buildLeadsTab(BookingState bookingState) {
    final currentUser = bookingState.getCurrentUserName();
    final pendingLeads = bookingState.bookings.where((b) => b.status == 'Pending' && b.workerName == currentUser).toList();
    final activeLeads = bookingState.bookings.where((b) => b.status != 'Pending' && b.workerName == currentUser).toList();
  ...
  Widget _buildLeadCard(BookingModel lead) {
    final statusColor = lead.status == 'Pending'
        ...
        : (lead.status == 'Accepted'
            ? Colors.blue
            : (lead.status == 'On the Way'
                ? Colors.teal
                : (lead.status == 'In Progress'
                    ? Colors.indigo
                    : (lead.status == 'Completed' ? Colors.green : Colors.red))));
          ...
          child: Container(
            ...
            child: Row(
              children: [
                CircleAvatar(
                  radius: 36,
                  backgroundImage: ((worker.image.isNotEmpty ? worker.image : 'assets/images/worker_ramesh.png').startsWith('http')
                      ? NetworkImage(worker.image.isNotEmpty ? worker.image : 'assets/images/worker_ramesh.png')
                      : AssetImage(worker.image.isNotEmpty ? worker.image : 'assets/images/worker_ramesh.png')) as ImageProvider,
            ...
            child: ListView(
              children: [
                _buildProfileOption(Icons.edit_outlined, 'Edit Profile', () {
                  _showEditProfileDialog(context, worker);
                }),
  ...
  void _showEditProfileDialog(BuildContext context, WorkerModel worker) {
    ...
    String selectedImage = worker.image.isNotEmpty ? worker.image : 'assets/images/worker_ramesh.png';
      ...
      child: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            Container(
              ...
              child: Column(
                ...
                children: [
                  Row(
                    ...
                    children: [
                      ...
                      Container(
                        ...
                        decoration: BoxDecoration(
                          color: isSubscriptionActive ? Colors.teal.withOpacity(0.1) : Colors.red.withOpacity(0.1),
                          borderRadius: BorderRadius.circular(20),
                        ),
                        child: Text(
                          isSubscriptionActive ? 'ACTIVE' : 'INACTIVE',
                          style: TextStyle(
                            color: isSubscriptionActive ? Colors.teal : Colors.red,
                            fontWeight: FontWeight.bold,
                            fontSize: 10,
                          ),
                        ),
                  ...
                  const SizedBox(height: 6),
                  Text(
                    isSubscriptionActive ? activePlan : 'No Active Plan',
                  ...
                  Row(
                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                    children: [
                      const Text('Price', style: TextStyle(color: AppTheme.textLight, fontSize: 13)),
                      Text(
                        isSubscriptionActive
                            ? (activePlan == 'Professional Plan'
                                ? '₹299 / Month'
                                : (activePlan == 'Premium Plan' ? '₹599 / Month' : '₹999 / Month'))
                            : '₹0',
                  ...
                  Row(
                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                    children: [
                      const Text('Remaining Days', style: TextStyle(color: AppTheme.textLight, fontSize: 13)),
                      Text(
                        isSubscriptionActive ? '$daysLeft Days' : '0 Days',
                        style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 13, color: AppTheme.textDark),
                      ),
                    ],
                  ),
            ...
            const SizedBox(height: 12),
            if (!isSubscriptionActive)
              _buildUpgradeCard('Professional Plan', '₹299 / Month', ['Unlimited Leads', 'Higher Visibility', 'Priority Support']),
            if (!isSubscriptionActive || activePlan == 'Professional Plan')
              _buildUpgradeCard('Premium Plan', '₹599 / Month', ['Unlimited Leads', 'Top Ranking', 'Priority Support', 'Featured Profile']),
            if (!isSubscriptionActive || activePlan == 'Professional Plan' || activePlan == 'Premium Plan')
              _buildUpgradeCard('Elite Plan', '₹999 / Month', ['Unlimited Leads', 'Featured Profile', 'Dedicated Business Account', 'Priority Support']),
            if (isSubscriptionActive && activePlan == 'Elite Plan')
              const Center(
                child: Padding(
                  padding: EdgeInsets.symmetric(vertical: 24.0),
                  child: Text(
                    'You are subscribed to our highest plan!',
          ...
          ElevatedButton(
            onPressed: () {
              Navigator.pushNamed(
                context,
                '/payment',
                arguments: {'title': name, 'price': price},
              );
            },
            style: ElevatedButton.styleFrom(
              backgroundColor: Colors.teal,
  ...
  Widget build(BuildContext context) {
    final bookingState = _bookingState;
    final currentUser = bookingState.getCurrentUserName();
    final recentLeads = bookingState.bookings.where((b) => b.status == 'Pending' && b.workerName == currentUser).toList();
    final totalLeads = bookingState.bookings.where((b) => b.workerName == currentUser).length;
