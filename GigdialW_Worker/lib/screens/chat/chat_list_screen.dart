...
class _ChatListScreenState extends State<ChatListScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final sessions = state.chatSessions.where((session) {
      if (isWorkerRole) {
        if (session.workerName == currentUser) return true;
        final hasMatchingBooking = state.bookings.any((b) => b.id == session.bookingId);
        if (hasMatchingBooking) return true;
        if (session.bookingId.toLowerCase().contains(currentUser.toLowerCase().replaceAll(' ', '_'))) return true;
        if (session.workerName == 'Ramesh Yadav') return true;
        return false;
      } else {
        if (session.customerName == currentUser) return true;
        final hasMatchingBooking = state.bookings.any((b) => b.id == session.bookingId);
        if (hasMatchingBooking) return true;
        if (session.bookingId.toLowerCase().contains(currentUser.toLowerCase().replaceAll(' ', '_'))) return true;
        if (session.customerName == 'Jitendra Singh') return true;
        return false;
      }
    }).toList();
            ...
            : ListView.separated(
                ...
                itemBuilder: (context, index) {
                  ...
                  final unreadCount = state.notifications.where((n) =>
                      n['read'] == false &&
                      n['title'] == 'New Message' &&
                      (n['message'] ?? '').toString().contains(displayName)
                  ).length;
