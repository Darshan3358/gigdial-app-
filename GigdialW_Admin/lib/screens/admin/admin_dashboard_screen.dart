...
class _AdminDashboardScreenState extends State<AdminDashboardScreen> {
  ...
  List<Map<String, dynamic>> get _subscriptions {
    final list = BookingState().paymentsList;
    if (list.isEmpty) {
      return [
        {'id': 'p1', 'worker': 'Sanjay Sharma', 'plan': 'Gold (Quarterly)', 'amount': '₹599', 'method': 'UPI', 'status': 'Success', 'color': Colors.green, 'rawStatus': 'success'},
        {'id': 'p2', 'worker': 'Mahesh Patel', 'plan': 'Silver (Monthly)', 'amount': '₹199', 'method': 'UPI', 'status': 'Pending Approval', 'color': Colors.orange, 'rawStatus': 'pending'},
        {'id': 'p3', 'worker': 'Ramesh Yadav', 'plan': 'Platinum (Yearly)', 'amount': '₹1,999', 'method': 'Net Banking', 'status': 'Success', 'color': Colors.green, 'rawStatus': 'success'},
      ];
    }
    return list.map((p) {
      final workerId = p['workerUid'] ?? '';
      final w = BookingState().allWorkersList.firstWhere(
        (worker) => worker.id == workerId,
        orElse: () => WorkerModel(id: '', name: 'Partner', profession: '', experience: '', rating: 5.0, reviews: '', location: '', image: '', skills: [], about: '')
      );
      final workerName = w.name.isNotEmpty && w.name != 'Partner' ? w.name : 'Partner ($workerId)';
  ...
  double get _totalRevenue {
    final list = BookingState().paymentsList;
    if (list.isEmpty) return 2797.0; // Mock revenue if no DB payments yet
    return list
        ...
        children: [
          ...
          GridView.count(
            ...
            children: [
              ...
              _buildMetricCard('Total Revenue', '₹${_totalRevenue.toStringAsFixed(0)}', Icons.currency_rupee, Colors.green),
            ],
          ),
          ...
          _buildHeader(
            ...
            trailing: Wrap(
              ...
              children: [
                  ...
                  onPressed: () {
                    showDialog(
                      ...
                      builder: (context) => AlertDialog(
                        ...
                        content: const Text('Do you want to download the current payment logs in CSV format?'),
                        actions: [
                          TextButton(
                          ...
                          ElevatedButton(
                            ...
                            onPressed: () {
                              ...
                              ScaffoldMessenger.of(context).showSnackBar(
                                const SnackBar(
                                  content: Text('Report exported successfully! CSV file saved in Downloads.'),
                                  backgroundColor: Colors.teal,
                                ),
                              );
                            },
                ...
                _buildSearchBox('Search Partner/Plan'),
              ],
            ),
          ),
          const SizedBox(height: 20),
                            ...
                            DataCell(
                              sub['rawStatus'] == 'success'
                                  ? ElevatedButton(
                                      ...
                                      onPressed: () async {
                                        final paymentId = sub['id'];
                                        if (paymentId != null && paymentId.isNotEmpty && !paymentId.startsWith('p')) {
                                          await BookingState().refundPayment(paymentId);
                                        } else {
                                          setState(() {
                                            sub['status'] = 'Refunded';
                                            sub['color'] = Colors.red;
                                            sub['rawStatus'] = 'refunded';
                                          });
                                        }
                                        ScaffoldMessenger.of(context).showSnackBar(
